...
def dashboard(request):
    ...
    user_wishes = Item.objects.all().filter(wisher = request.session['loggedInId'] )
...
def make_wish(request):
    ...
    if len(errors) > 0:
        ...
        Item.objects.create(item = request.POST['item'], desc = request.POST['desc'], wisher = User.objects.get(id = request.session['loggedInId']))
    return redirect('/dashboard')
...
def wishes_stats(request):
    ...
    user_wishes = Item.objects.filter(wisher = request.session['loggedInId'])
